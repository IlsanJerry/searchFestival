 ...
 <nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-dark scrolling-navbar">
				...
				<%-- <h4>${resultlist.totalPages}, ${resultlist.startPage}, ${resultlist.endPage}</h4> --%>
					<table class="table table-stripaed" style="text-align: center; border: 1px solid #dddddd; height: 70%">
						<c:forEach var="vo" items="${resultlist.list }">
							<tr onclick="location.href='http://70.12.113.165:8000/festival/detail?fid=${vo.fid}'">
								<td width="40%">
							...
							<tr>
								<td>
									<c:if test="${resultlist.startPage > 5}">
										<a href="http://70.12.113.165:8000/festival/list?pageNo=${resultlist.startPage - 5}&action=">[이전]</a>
									</c:if> <c:forEach var="pNo" begin="${resultlist.startPage}" end="${resultlist.endPage}">
										<a href="http://70.12.113.165:8000/festival/list?pageNo=${pNo}&action=">[${pNo}]</a>
									</c:forEach> <c:if test="${resultlist.endPage < resultlist.totalPages}">
										<a href="http://70.12.113.165:8000/festival/list?pageNo=${resultlist.startPage + 5}&action=paging">[다음]</a>
									</c:if>
								</td>
							</tr>
						</c:if>
	...
	<script>
		...
		for ( var i in info) {
			lat = info[i][0] * 1;
			lng = info[i][1] * 1;
			L.marker([ lat, lng ], 	{icon : myIcon}).bindPopup('<a href="http://70.12.113.165:8000/festival/detail?fid='+info[i][3]+'">'+ info[i][2] + '</a>').addTo(map);
		}
});
	</script>
